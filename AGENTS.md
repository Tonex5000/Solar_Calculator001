...
## AI-backend (Layer 2 Solar Assistant)
- **Stack:** FastAPI (`main.py`) + two Groq-style service modules (`services/ai_extractor.py`, `services/ai_responder.py`). Single `/chat` endpoint + `/health`. Stateless per message (no conversation history).
- **LLM provider: NVIDIA NIM** (NOT Groq). Uses the OpenAI-compatible SDK (`openai` package, NOT `groq`). Both services construct `OpenAI(api_key=NVIDIA_API_KEY, base_url=NVIDIA_BASE_URL)` with base URL `https://integrate.api.nvidia.com/v1` and model `nvidia/nemotron-3.5-lightning-30b-a3b`. Both `NVIDIA_BASE_URL` and `NVIDIA_MODEL` are env-overridable with those defaults.
- **Env var:** `NVIDIA_API_KEY` (required at request time). `main.py` `/chat` guard raises 500 if missing. The client is constructed with `api_key=os.getenv("NVIDIA_API_KEY") or "not-configured"` so the app still imports/starts without a key (letting `/health` and the guard work); a real call fails at request time.
- **Pipeline (per /chat):** `extract_intent(message)` → JSON `{intent, data:{budget,appliances,problem,comparison}}` (temp 0.1, `response_format=json_object`) → `generate_reply(message, structured_data)` (temp 0.7). 5 intents: budget_planning, load_estimation, problem_diagnosis, cost_comparison, general_question. Validated against `SUPPORTED_INTENTS`, fallback general_question.
- **Deployment:** Render (`render.yaml` → `ai-solar-backend`, startCommand uvicorn port 10000; `Procfile` uses `$PORT`). Deps in `requirements.txt`: fastapi, uvicorn[standard], openai, python-dotenv (groq removed).
- **AI-frontend:** Vite + React 19 chat (`AI-frontend/src/components/AIEngineerPage.jsx`) — the Voltra AI-Engineer UI. Self-contained dark green NVIDIA-style chat: inline styles + a scoped `<style>` block own fonts/colors/layout/hover/typing animations; `src/index.css` is a minimal document reset only (no global theme classes). `askAIEngineer()` is a keyword-matching STUB (no backend wired yet) — pending decision to POST conversation history to a solar-backend `/ai-engineer/chat` endpoint or the AI-backend `/chat`. Old `Chat.jsx` + purple-gradient `index.css` were removed. `main.jsx` → `<AIEngineerPage />`. Title: "Voltra AI-Engineer".
