...
- **Modules:** `server.py` (FastMCP `voltra-product-db`, 14 read-only tools), `supabase_client.py` (lazy client, read-only queries), `design.py` (engineering-target→catalog composition; panel matching works, battery/inverter raise `SchemaNotConfirmedError` on purpose), `Engineering.py` (capital E — imported as `import Engineering`; deterministic cable sizing/safety checks), `peak_sun_hours.py` (NASA POWER climatology PSH lookup with Nominatim geocoding + regional fallback bands), `langchain_agent.py` (MultiServerMCPClient launches `server.py` as a stdio subprocess; run `python langchain_agent.py`). **`main.py`** — a FastAPI ASGI wrapper (`uvicorn main:app`) that re-exposes the same 14 MCP tools over HTTP (`GET /health`, `GET /tools`, `GET|POST /tools/{name}`). It ALSO serves **POST /chat**, which lazily builds the LangChain ReAct agent (the same one `langchain_agent.py` uses) and runs the user's message through it — this is what the AI-frontend calls (accepts `{"message"}` or `{"text"}`, returns `{reply}`). Without `NVIDIA_API_KEY`, `/chat` returns 503 while everything else works. CORS middleware added (browser frontend cross-origin). Render config in `render.yaml`.
